...
# Experimental variant jisme tools ek MCP server se aate hain (external ERPNext tool server). 
# CPU slow hai isliye timeouts bade aur result 1500 chars pe cap. ask_mcp iska whitelisted API wrapper hai. 
def mcp_agent_answer(question, context=None, max_steps=6):
    """
    Agent loop jisme tools MCP ERPNext server (@casys/mcp-erpnext) se aate hain.
    Tool schemas MCP tools/list se, execution MCP tools/call se.
    """
    from classic_chatbot.api.mcp_client import get_ollama_tools, call_mcp_tool

    context = context or {}
    tools = get_ollama_tools(question=question)

    # CPU inference slow hai (~14 tok/s prompt eval) - prompt jitna chhota utna fast
    user_msg = question
    if context.get("doctype"):
        user_msg += f"\n(active screen: {context['doctype']})"

    messages = [
        {
            "role": "system",
            "content": (
                "You are an ERP assistant with live ERPNext tools. "
                "erpnext_doc_list works for ANY DocType (User, Employee, Purchase Order, "
                "Supplier, Lead, Task...) with filters like [[\"field\",\"like\",\"%value%\"]]. "
                "Use tools for exact data, use limit 5 unless asked for more. "
                "Never invent data. Reply in Hinglish (Roman script), max 2 short sentences."
            ),
        },
    ...
    tools_called = []
    for step in range(max_steps):
        # CPU-only inference me tool schemas ke sath prompt eval slow hai
        msg = call_ollama_with_tools(messages, tools=tools, timeout=300)
        tool_calls = msg.get("tool_calls") or []
        ...
        for tc in tool_calls:
            ...
            try:
                result = call_mcp_tool(fn, args)
            except Exception as e:
                result = json.dumps({"error": str(e)})

            tools_called.append({"tool": fn, "args": args})
            # 1500 chars cap: bada result CPU par prompt eval me minutes kha jata hai
            messages.append({
                "role": "tool",
                "content": str(result)[:1500],
            })

    return "Bhai, is question ke liye kaafi steps lag rahe hain, thoda specific pucho.", {
        "steps": max_steps,
...
@frappe.whitelist()
def ask_mcp(question, doctype=None, docname=None, doc=None, route=None, error=None, preferred_model="auto"):
    """MCP-backed variant of ask(). Frontend me method badal kar test kar sakte ho."""
    if not question:
        frappe.throw("Question is required")
    ...
    try:
        answer, debug = mcp_agent_answer(question, context)
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Classic Chatbot MCP Agent Error")
        return {
            "answer": f"Bhai, MCP agent me error aaya: {str(e)}",
            "model_used": "⚠️ Error",
            "tools_used": [],
        }
    ...
    return {
        "answer": answer,
        "model_used": "🔌 MCP ERPNext Agent (Local)",
        "tools_used": [t["tool"] for t in (debug or {}).get("tools_called", [])],
